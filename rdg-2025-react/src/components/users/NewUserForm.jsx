...
const NewUserForm = () => {
  ...
  const roleOptions = [{ value: "admin", label: "Admin" }, { value: "superadmin", label: "Super Admin" }];
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    const response = await UserService.createUser(name, email, role.value, "password")
  ...
  return (
    <ContentCard>
      ...
      <form className="flex flex-col gap-2 max-w-md" onSubmit={handleSubmit}>
        ...
        <ConfirmCancelButtons handleCancel={handleCancel} disableSubmit={disableSubmit} />
      </form>
    </ContentCard>
  );
};
