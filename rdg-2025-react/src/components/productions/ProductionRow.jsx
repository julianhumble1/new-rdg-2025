...
const ProductionRow = ({
  ...
  useEffect(() => {
    const nameMatches = production.name
      .toLowerCase()
      .includes(nameSearch.toLowerCase());
    const venueMatches =
      production.venue &&
      production.venue.name.toLowerCase().includes(venueSearch.toLowerCase());

    if (sundownersSearch && !production.sundowners) {
      setHide(true);
  ...
  return (
    <Table.Row
      className={`bg-white dark:border-gray-700 dark:bg-gray-800 ${hide && "hidden"}`}
    >
      ...
      <Table.Cell>{production.sundowners ? "Yes" : "No"}</Table.Cell>
      <Table.Cell>{production.author}</Table.Cell>
      <Table.Cell>
        <div className="flex gap-2">
          <Link
            className="text-medium text-black hover:underline font-bold text-end"
            to={`/productions/${production.id}?edit=true`}
          >
            Edit
          </Link>
          <button
