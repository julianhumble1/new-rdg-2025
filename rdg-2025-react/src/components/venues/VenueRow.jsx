...
const VenueRow = ({ venue, handleDelete, nameSearch }) => {
  ...
  return (
    ...
    >
      ...
      <Table.Cell>{venue.town}</Table.Cell>
      <Table.Cell>{venue.postcode}</Table.Cell>
      <Table.Cell>{format(new Date(venue.createdAt), "dd-MM-yyyy")}</Table.Cell>
      <Table.Cell>
        <div className="flex gap-2">
          <Link
            className="text-medium text-black hover:underline font-bold text-end"
            to={`/venues/${venue.id}?edit=true`}
          >
            Edit
          </Link>
          <button
