    ...
    useEffect(() => {
        const getAwardById = async () => {
            try {
                ...
                setProduction(response.data.award.production ?
                    { value: response.data.award.production.id, label: response.data.award.production.name } : null
                )
                setPerson(response.data.award.person ?
                    { value: response.data.award.person.id, label: `${response.data.award.person.firstName} ${response.data.award.person.lastName}` } : null
                )
                setFestival(response.data.award.festival ?
                    { value: response.data.award.festival.id, label: `${response.data.award.festival.name} (${response.data.award.festival.year})` } : null
                )
            } catch (e) {
                setErrorMessage(e.message)
            }
            ...
            <div className="bg-sky-900 bg-opacity-35 lg:w-1/2 md:w-2/3 rounded p-4 m-2 flex flex-col gap-2 shadow-md">
                <form className="flex flex-col gap-2 max-w-md" onSubmit={handleSubmit} >
                    <div>
                        ...
                        <TextInput placeholder="Best Actor" required value={name} onChange={(e) => setName(e.target.value)} />
                    </div>
                    <div>
                        <div className="mb-2 block">
                            <Label value="Person" />
                        </div>
                        <Select options={personOptions} value={person} onChange={setPerson} isClearable className="w-full text-sm" styles={{ control: (baseStyles) => ({ ...baseStyles, borderRadius: 8, padding: 1 }) }} />
                    </div>
                    <div>
                        <div className="mb-2 block">
                            <Label value="Production" />
                        </div>
                        <Select options={productionOptions} value={production} onChange={setProduction} isClearable className="w-full text-sm" styles={{ control: (baseStyles) => ({ ...baseStyles, borderRadius: 8, padding: 1 }) }} />
                    </div>
                    <div>
                        <div className="mb-2 block">
                            <Label value="Festival" />
                        </div>
                        <Select options={festivalOptions} value={festival} onChange={setFestival} isClearable className="w-full text-sm" styles={{ control: (baseStyles) => ({ ...baseStyles, borderRadius: 8, padding: 1 }) }} />
                    </div>
