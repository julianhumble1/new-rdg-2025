    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await AwardService.createNewAward(
                name, production.value, person.value, festival.value
            )
            setSuccessMessage(`Successfully added award "${response.data.award.name}"!`)
        } catch (e) {
            setErrorMessage(e.message)
        }
    }
            ...
            <form className="flex flex-col gap-2 max-w-md" onSubmit={handleSubmit}>
                ...
                <div>
                    <div className="mb-2 block">
                        <Label value="Person (required)" />
                    </div>
                    <Select options={personOptions} value={person} required onChange={setPerson} className="w-full text-sm" styles={{ control: (baseStyles) => ({ ...baseStyles, borderRadius: 8, padding: 1 }) }} />
                </div>
                <div>
                    <div className="mb-2 block">
                        <Label value="Production (required)" />
                    </div>
                    <Select options={productionOptions} value={production} required onChange={setProduction} className="w-full text-sm" styles={{ control: (baseStyles) => ({ ...baseStyles, borderRadius: 8, padding: 1 }) }} />
                </div>
                <div>
                    <div className="mb-2 block">
                        <Label value="Festival (required)" />
                    </div>
                    <Select options={festivalOptions} value={festival} required onChange={setFestival} className="w-full text-sm" styles={{ control: (baseStyles) => ({ ...baseStyles, borderRadius: 8, padding: 1 }) }} />
                </div>
