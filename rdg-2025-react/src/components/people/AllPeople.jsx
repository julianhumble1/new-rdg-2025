...
const AllPeople = () => {
	...
	const handleConfirmDelete = async (person) => {
		try {
			...
			setSuccessMessage(`Successfully deleted ${person.firstName} ${person.lastName}`)
			fetchAllPeople()
		} catch (e) {
			setErrorMessage(e.message)
		}
    ...
    return (
		<div>
			{showConfirmDelete &&
                <ConfirmDeleteModal setShowConfirmDelete={setShowConfirmDelete} itemToDelete={personToDelete} handleConfirmDelete={ handleConfirmDelete } />
            }
            ...
            <div className='grid lg:grid-cols-6 grid-cols-1'>
                ...
                <span className='col-span-5 p-2 pl-0 overflow-x-auto'>
                    <PeopleTable people={people} responseType={responseType } handleDelete={handleDelete}/>
				</span>
            </div>
		</div>
  	)
