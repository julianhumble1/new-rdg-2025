...
const FestivalRow = ({ festival, handleDelete, nameSearch, venueSearch }) => {
  ...
  useEffect(() => {
    const nameMatches = festival.name
      .toLowerCase()
      .includes(nameSearch.toLowerCase());
    const venueMatches =
      festival.venue &&
      festival.venue.name.toLowerCase().includes(venueSearch.toLowerCase());
      ...
      <Table.Cell>
        <div className="flex gap-2">
          <Link
            ...
            to={`/festivals/${festival.id}?edit=true`}
          >
            Edit
          </Link>
          <button
