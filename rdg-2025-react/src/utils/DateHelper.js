...
export default class DateHelper {
  ...
  static createPerformanceStatement = (performances) => {
    ...
    const performanceDates = performances.map(
      (performance) => performance.time,
    );
    performanceDates.sort((a, b) => new Date(a) - new Date(b));
    const firstPerformanceString = format(
      performanceDates[0],
      "MMMM d, yyyy, h:mm a",
    );
    const lastPerformanceString = format(
      performanceDates[performanceDates.length - 1],
      "MMMM d, yyyy, h:mm a",
    );
